...
export default function AllTimeStats() {
  ...
  return (
    <Card className="stadium-card overflow-hidden relative">
      <div className="absolute top-0 right-0 w-64 h-64 bg-gradient-to-br from-cricket-gold/10 to-primary/10 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 pointer-events-none" />
      ...
      <CardContent className="p-6 relative">
        <Tabs defaultValue="batting" className="w-full">
          <TabsList className="grid w-full grid-cols-2 mb-6 h-14 p-1.5 bg-muted/50">
            <TabsTrigger value="batting" className="gap-2.5 text-sm font-bold data-[state=active]:bg-gradient-to-r data-[state=active]:from-orange-500 data-[state=active]:to-orange-600 data-[state=active]:text-white data-[state=active]:shadow-lg transition-all duration-300">
              <TrendingUp className="h-4 w-4" /> Batting Leaders
            </TabsTrigger>
            <TabsTrigger value="bowling" className="gap-2.5 text-sm font-bold data-[state=active]:bg-gradient-to-r data-[state=active]:from-purple-500 data-[state=active]:to-purple-600 data-[state=active]:text-white data-[state=active]:shadow-lg transition-all duration-300">
              <Target className="h-4 w-4" /> Bowling Leaders
            </TabsTrigger>
          </TabsList>
                  ...
                  return (
                    <div key={player.id} onClick={() => navigate(`/player/${player.player_id}`)}
                      className={cn("grid grid-cols-12 gap-2 items-center p-3 rounded-xl transition-all duration-300 cursor-pointer group animate-fade-slide-up",
                        index < 3 ? "bg-gradient-to-r from-orange-500/10 via-orange-500/5 to-transparent border-2 border-orange-500/20 hover:border-orange-500/40 hover:shadow-lg hover:shadow-orange-500/10"
                      ...
                      <div className="col-span-3 flex items-center gap-3">
                        <div className="relative">
                          <Avatar className={cn("h-10 w-10 border-2 transition-all", index === 0 ? "border-yellow-500 ring-2 ring-yellow-500/30" : "border-orange-500/30")}>
                            ...
                            <AvatarFallback className="bg-orange-500/10 text-orange-600"><User className="h-5 w-5" /></AvatarFallback>
                          </Avatar>
                      ...
                      <div className="col-span-2 text-center"><span className="font-black text-xl text-orange-600">{player.total_runs}</span></div>
              ...
              <div className="space-y-2">
                ...
                {bowlingLeaderboard.slice(0, 15).map((player, index) => {
                  ...
                  return (
                    <div key={player.id} onClick={() => navigate(`/player/${player.player_id}`)}
                      className={cn("grid grid-cols-12 gap-2 items-center p-3 rounded-xl transition-all duration-300 cursor-pointer group animate-fade-slide-up",
                        index < 3 ? "bg-gradient-to-r from-purple-500/10 via-purple-500/5 to-transparent border-2 border-purple-500/20 hover:border-purple-500/40 hover:shadow-lg hover:shadow-purple-500/10"
                          : "bg-card/50 hover:bg-muted/50 border border-transparent hover:border-border"
                      )} style={{ animationDelay: `${index * 40}ms` }}>
                      <div className="col-span-1">
                        <span className={cn("w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold transition-transform group-hover:scale-110", getMedalClass(index))}>
                      ...
                      <div className="col-span-3 flex items-center gap-3">
                        <div className="relative">
                          <Avatar className={cn("h-10 w-10 border-2 transition-all", index === 0 ? "border-purple-500 ring-2 ring-purple-500/30" : "border-purple-500/30")}>
                            <AvatarImage src={player.image_url || undefined} alt={player.player_name} />
                            <AvatarFallback className="bg-purple-500/10 text-purple-600"><User className="h-5 w-5" /></AvatarFallback>
                          </Avatar>
                          {index === 0 && <div className="absolute -top-1 -right-1 w-4 h-4 bg-purple-500 rounded-full flex items-center justify-center"><Crown className="h-2.5 w-2.5 text-white" /></div>}
                        </div>
                        <div className="min-w-0">
                      ...
                      <div className="col-span-2 text-center"><span className="font-black text-xl text-purple-600">{player.total_wickets}</span></div>
